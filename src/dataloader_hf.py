...
class Flickr30kEvalDataset(Dataset):
    def __init__(self, df, vocab, image_root, transform=None):
        ...
        self.samples = []
        for img_id, row in df.reset_index(drop=True).iterrows():
            captions = ast.literal_eval(row["raw"])
            for cap in captions:
    ...
    def encode_caption(self, caption):
        tokens = tokenize(caption)
        ids = [self.sos_idx] + \
              [self.word2int.get(t, self.unk_idx) for t in tokens] + \
              [self.eos_idx]
        return torch.tensor(ids, dtype=torch.long)
    ...
    def __getitem__(self, idx):
        ...
        caption_ids = self.encode_caption(caption)
        return caption_ids, img_id, self.pad_idx


# commented out old eval dataset class
# class Flickr30kEvalDataset(Dataset):
#     def __init__(self, df, vocab, image_root, transform=None):
#         self.vocab = vocab
#         self.image_root = image_root
...
# commented out old eval collate function
# def eval_collate_fn(batch):
#     images, captions, img_ids, pad_idxs = zip(*batch)
#     pad_idx = pad_idxs[0]
...
def eval_collate_fn(batch):
    captions, img_ids, pad_idxs = zip(*batch)
    pad_idx = pad_idxs[0]

    lengths = [len(c) for c in captions]
    captions = pad_sequence(captions, batch_first=True, padding_value=pad_idx)

    return captions, img_ids, lengths
